...
const ProductCard = ({ product }) => {
  const { addToCart, wishlist, toggleWishlist, user } = useContext(StoreContext);
    ...
    <div className="bg-white shadow-md rounded-xl p-4 relative flex flex-col justify-between h-[360px]">
      ...
      {user && user.role === "Customer" && (
        <button
          ...
          className="absolute top-6 right-6 bg-white p-1.5 rounded-full shadow hover:scale-115 transition text-red-500 z-10"
        >
      ...
      {/* BUTTON (Only show add to cart for Customers or guest users) */}
      {(!user || user.role === "Customer") ? (
        product.stock > 0 ? (
          <button
